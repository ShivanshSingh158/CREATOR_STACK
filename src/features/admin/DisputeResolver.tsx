...
import {
  ...
  MessageSquare, ShieldCheck, FileText, Scale,
} from 'lucide-react';
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; cls: string }> = {
    ...
    resolved_creator: { label: '✅ Resolved — Creator Wins', cls: 'bg-emerald-100 text-emerald-800' },
    resolved_split: { label: '🤝 Resolved — Split', cls: 'bg-blue-100 text-blue-800' },
  };
  const s = map[status] || { label: status, cls: 'bg-gray-100 text-gray-700' };
  return (
    <span className={`text-[10px] font-black px-2 py-1 rounded-md border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] ${s.cls}`}>
      {s.label}
    </span>
  );
}
      ...
      <div className="bg-white border-b-2 border-black px-6 lg:px-10 py-5">
        <div className="max-w-5xl mx-auto flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Manage disputed deals</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="bg-indigo-50 border-2 border-black rounded-xl p-5 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
          ...
          <ol className="text-xs font-medium text-indigo-800 space-y-1 list-decimal list-inside leading-relaxed">
            ...
            <li>Resolution options: full release to creator, full refund to brand, or a negotiated split.</li>
          </ol>
        </div>
          ...
          <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white border-2 border-black rounded-2xl shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] w-full max-w-lg p-8">
              ...
              <p className="text-xs font-medium text-gray-600 mb-6">
                Describe your case. Include links to the content, timestamps, communication records, or any other proof.
              </p>
              <textarea
              ...
              <div className="flex gap-3 mt-5">
                <button onClick={() => setSelected(null)} className="flex-1 py-3 text-xs font-black uppercase tracking-widest border-2 border-black bg-white rounded-xl shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 transition-all">
                  Cancel
                </button>
                <button
        ...
        {submitted && (
          <div className="bg-emerald-50 border-2 border-emerald-400 rounded-xl p-4 flex items-center gap-3 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
            ...
            <p className="text-sm font-black text-emerald-900">Evidence submitted! The CreatorStack team will review within 48 hours.</p>
            <button onClick={() => setSubmitted(false)} className="ml-auto text-emerald-600 hover:text-emerald-800 font-black text-sm">✕</button>
          </div>
        )}
        ...
        <div>
          ...
          {openDisputes.length === 0 ? (
            <div className="bg-white border-2 border-dashed border-gray-300 rounded-xl p-10 text-center">
              ...
              <p className="text-sm font-black text-gray-400 uppercase tracking-widest">No open disputes</p>
            </div>
          ) : (
            <div className="space-y-4">
              {openDisputes.map(d => (
                <div key={d.id} className="bg-white border-2 border-black rounded-xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] p-6">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <StatusBadge status={d.status} />
                        <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Deal: {d.dealId}</span>
                      </div>
                      ...
                      {d.evidence && (
                        <div className="bg-amber-50 border-2 border-amber-300 rounded-lg p-3 mt-3">
                          <p className="text-[10px] font-black text-amber-700 uppercase tracking-widest mb-1">Evidence Submitted</p>
        ...
        {resolvedDisputes.length > 0 && (
          <div>
            <h2 className="text-sm font-black text-black uppercase tracking-widest mb-3 flex items-center gap-2">
              <CheckCircle2 className="w-4 h-4 text-emerald-500" /> Resolved ({resolvedDisputes.length})
            </h2>
            <div className="space-y-3">
              {resolvedDisputes.map(d => (
                <div key={d.id} className="bg-white border-2 border-gray-300 rounded-xl p-5 opacity-70">
                  <div className="flex items-center justify-between gap-4">
                    <p className="text-sm font-bold text-gray-700">{d.reason}</p>
                    <StatusBadge status={d.status} />
                  </div>
                  {d.resolution && <p className="text-xs font-medium text-gray-500 mt-2">{d.resolution}</p>}
                </div>
              ))}
            </div>
          </div>

...
export default function PanVerification({ onVerified }: PanVerificationProps) {
  ...
  if (submitted) {
    return (
      <div className="bg-[#a3e635] border-2 border-black rounded-lg p-4 flex items-center gap-3">
        ...
        <div>
          <p className="text-xs font-black text-black uppercase tracking-widest">PAN Format Valid</p>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <button type="submit" className="w-full bg-indigo-600 border-2 border-black text-white font-black py-3.5 rounded-lg uppercase tracking-widest text-[10px] hover:bg-indigo-700 hover:-translate-y-0.5 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] transition-all">
        Submit for Review
      </button>
    </form>
  );

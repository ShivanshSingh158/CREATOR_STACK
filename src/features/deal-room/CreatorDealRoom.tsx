...
export default function CreatorDealRoom() {
  ...
  const handleSignContract = async () => {
    ...
    runAnimation(
      [
        'Verifying identity via CreatorStack Auth...',
        'Applying cryptographic signature...',
        'Notifying brand counterparty...',
        'Contract co-executed.',
      ],
      async () => {
        try {
          await setDoc(
      ...
      >
        <div className="bg-indigo-600 px-8 py-6 flex justify-between items-center border-b-2 border-black">
          <div>
            <p className="inline-flex items-center gap-3 text-3xl font-black text-white tracking-tight">
              <img src="/logo.png" alt="Logo" className="w-8 h-8 object-contain bg-white rounded-md p-1 border-2 border-black" />
          ...
          <div className="text-right">
            ...
            <p className="text-indigo-200 font-mono text-sm mt-1">
              CS-INV-{campaignId?.substring(0, 6).toUpperCase()}-{new Date().getFullYear()}
            </p>
        ...
        <div className="p-6">
          <div className="grid grid-cols-2 gap-8 mb-6 pb-6 border-b border-[#e5e7eb]">
            <div>
              ...
              <p className="text-sm text-[#6b7280]">
                PAN:{' '}
                {creatorProfile?.pan
                  ? `${creatorProfile.pan.substring(0, 3)}****${creatorProfile.pan.substring(7)}`
                  : 'Verified via Signzy'}
              </p>
              <p className="text-sm text-[#6b7280]">
                UPI: {creatorProfile?.upi || 'Verified UPI on file'}
              </p>
            </div>
            <div>
              <p className="text-xs font-bold text-[#9ca3af] uppercase tracking-wider mb-3">
              ...
              <p className="text-sm text-[#6b7280]">
                Campaign: {dealRoom?.campaignTitle || campaign?.title}
              </p>
          ...
          <table className="w-full mb-8">
            <thead>
              <tr className="bg-[#f9fafb] border-y border-[#e5e7eb]">
                <th className="text-left text-xs font-bold text-[#6b7280] uppercase tracking-wider px-4 py-3">
                  Description of Service
                </th>
                <th className="text-right text-xs font-bold text-[#6b7280] uppercase tracking-wider px-4 py-3">
                  HSN/SAC
                </th>
                <th className="text-right text-xs font-bold text-[#6b7280] uppercase tracking-wider px-4 py-3">
                  Amount (₹)
            ...
            <tbody>
              <tr className="border-b border-[#f3f4f6]">
                <td className="px-4 py-4">
                  ...
                  <p className="text-sm text-[#6b7280]">
                    "{dealRoom?.campaignTitle || campaign?.title}" — {dealRoom?.productionDays}-day
                    production, ASCI-compliant
                  </p>
                </td>
                <td className="px-4 py-4 text-right text-sm text-[#6b7280] font-mono">998361</td>
                <td className="px-4 py-4 text-right font-bold text-[#111827]">
                  {formatRupee(gross)}
                </td>
              </tr>
            </tbody>
          ...
          <div className="bg-amber-50 border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] p-4 rounded-lg text-xs text-black mb-6 font-medium">
            <strong>TDS Withholding Note:</strong> TDS under Section 194J has been deposited to the
            Income Tax Department against PAN {creatorProfile?.pan || 'verified on file'}. This is a
          ...
          <div className="flex gap-4">
            ...
            <button
              ...
              className="flex-1 border border-[#e5e7eb] text-[#111827] font-bold py-3 rounded-lg hover:bg-[#f9fafb] transition-colors flex items-center justify-center gap-2"
            >
    ...
    return (
      <div className="min-h-[calc(100vh-75px)] bg-[#f9fafb] flex flex-col items-center justify-center text-center p-8">
        ...
        <p className="text-gray-600 mb-8 max-w-md">
          The brand hasn't opened a deal room for this campaign yet. You'll be notified when they
          initiate a contract.
        </p>
        <button
          onClick={() => navigate('/creator-dashboard')}
          className="flex items-center gap-2 text-indigo-700 bg-indigo-50 border-2 border-black hover:-translate-y-0.5 hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-y-0 active:shadow-none px-6 py-3 rounded-lg font-bold transition-all"
        >
          ...
          <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
            <div className="shrink-0">
              ...
              <p className="mt-0.5 text-sm text-gray-600 max-w-2xl font-medium">
                ...
                {dealRoom?.brandName || campaign?.brandName}
              </p>
            </div>
            ...
            <div className="flex gap-4 items-center shrink-0">
              <div
                className={`flex items-center gap-2 px-4 py-2 rounded-lg border-2 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] font-bold text-sm ${
                  status === 'completed'
                    ? 'bg-emerald-50 border-black text-emerald-700'
                    : status === 'escrow_locked' || status === 'pod_submitted'
                      ? 'bg-sky-50 border-black text-sky-700'
          ...
          <div className="bg-white border-2 border-black rounded-2xl w-full max-w-md shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            <div className="px-7 py-5 border-b-2 border-black flex justify-between items-center bg-gray-50 rounded-t-xl">
              <h2 className="text-lg font-black text-black flex items-center gap-2">
                <PenLine className="w-5 h-5 text-indigo-600" /> Digital Signature
              </h2>
              {!actionLoading && (
                <button
                  onClick={() => setShowSignModal(false)}
                  className="text-gray-500 hover:text-black text-xl font-black transition-colors"
                >
                  ✕
                </button>
              )}
            ...
            {animating ? (
              ...
              <div className="p-7 space-y-5">
                <div className="bg-amber-50 border-2 border-black rounded-xl p-4 text-xs font-bold text-amber-800 leading-relaxed shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                  By signing below, you confirm you have read and agree to all terms. This
                  constitutes a legally binding digital signature under the IT Act, 2000.
                </div>
                <div>
                ...
                {signatureInput.trim() && (
                  <div className="bg-gray-50 border-2 border-black rounded-lg p-4 text-center shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                    <p className="text-[10px] text-gray-500 mb-2 uppercase tracking-wider font-bold">
                      Preview
                    </p>
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    onClick={() => setShowSignModal(false)}
                    className="flex-1 py-3 text-sm font-black text-gray-700 bg-white border-2 border-black rounded-xl hover:bg-gray-50 transition-all shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 active:translate-y-0 active:shadow-none"
                  >
                    Cancel
                  </button>
                  <button

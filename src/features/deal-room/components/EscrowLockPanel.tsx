...
export default function EscrowLockPanel({ dealAmount, onLock, locked = false }: EscrowLockPanelProps) {
  ...
  if (locked) {
    return (
      <div className="bg-[#a3e635] border-2 border-black rounded-xl p-5 text-center shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
        ...
        <p className="text-[10px] font-bold text-black/70 uppercase tracking-widest mt-1">{format(dealAmount)} earmarked</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white border-2 border-black rounded-xl p-5 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
      <div className="flex justify-between items-center mb-4">
        <p className="text-[10px] font-black text-black uppercase tracking-widest">Wallet Balance</p>
      ...
      {!hasEnough && (
        <div className="bg-amber-50 border border-amber-400 rounded-lg p-3 mb-4 text-[9px] font-black uppercase tracking-widest text-amber-800">
          ...
          <button onClick={() => navigate('/wallet')} className="block mt-1 text-indigo-600 underline">
            Deposit funds →
          </button>
        </div>
      )}

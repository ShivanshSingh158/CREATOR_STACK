...
export default function BrandDashboard() {
  ...
  const filteredCampaigns = useMemo(() => {
    ...
    if (statusFilter !== 'All') {
      filtered = filtered.filter((c) =>
        statusFilter === 'Active' ? c.status !== 'completed' : c.status === 'completed',
      );
    }
    return filtered;
  ...
  const displayName =
    profileData?.companyName || profileData?.name || currentUser?.email?.split('@')[0] || 'Brand';
      ...
      <div className="flex min-h-[calc(100vh-64px)] max-w-screen-2xl mx-auto">
        ...
        <aside className="hidden xl:flex w-64 shrink-0 flex-col sticky top-16 h-[calc(100vh-64px)] overflow-y-auto px-5 py-6">
          ...
          <div className="space-y-1 flex-1">
            ...
            <div className="text-[10px] font-bold text-gray-500 uppercase tracking-widest px-2 mt-8 mb-2">
              Stats
            </div>
        ...
        <main className="flex-1 min-w-0">
          ...
          <div className="xl:hidden grid grid-cols-2 sm:grid-cols-4 gap-4 p-5 bg-white border-b-2 border-black">
            {[
              ...
              {
                label: 'Reach',
                value: totalReach > 0 ? formatNumberCompact(totalReach) : '0',
                icon: TrendingUp,
              },
            ...
            {loading ? (
              ...
              <div className="bg-white rounded-2xl border-2 border-dashed border-gray-300 p-10 text-center shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                ...
                <p className="text-[10px] font-bold text-gray-500 mb-6 uppercase tracking-widest">
                  Create your first campaign to start working with verified creators.
                </p>
                <Link
                  to="/create-campaign"
                  className="inline-flex items-center gap-2 bg-indigo-600 text-white text-[10px] font-black uppercase tracking-widest border-2 border-black px-4 py-2.5 rounded-lg shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 hover:bg-indigo-700 active:translate-y-0 active:shadow-none transition-all"
                >
              ...
              <div className="bg-white rounded-xl border-2 border-dashed border-gray-300 p-12 text-center">
                ...
                <button
                  ...
                  className="mt-4 text-sm font-black text-indigo-600 hover:text-indigo-800 uppercase tracking-widest hover:underline"
                >
                  Clear filters
                </button>
              </div>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b-2 border-black bg-indigo-50">
                        <th className="text-left px-6 py-4 text-xs font-black text-indigo-900 uppercase tracking-widest">
                          Campaign
                        </th>
                        <th className="text-left px-4 py-4 text-xs font-black text-indigo-900 uppercase tracking-widest">
                          Niche
                        </th>
                        <th className="text-left px-4 py-4 text-xs font-black text-indigo-900 uppercase tracking-widest">
                          Budget
                        ...
                        return (
                          ...
                          >
                            <td className="px-6 py-4">
                              <p className="font-black text-black uppercase tracking-tight group-hover:text-indigo-600 transition-colors">
                                {campaign.title}
                              </p>
                              ...
                              <div className="flex flex-wrap gap-1.5">
                                {Array.isArray(campaign.niche) ? (
                                  campaign.niche.map((n: string, idx: number) => (
                                    <span
                                      key={idx}
                                      className="text-[10px] font-bold text-gray-600 bg-gray-100 border border-gray-200 px-2 py-1 rounded uppercase tracking-widest"
                                    >
                                      {n}
                                    </span>
                                  ))
                                ) : (
                                  <span className="text-[10px] font-bold text-gray-600 bg-gray-100 border border-gray-200 px-2 py-1 rounded uppercase tracking-widest">
                                    {campaign.niche}
                                  </span>
                                )}
                              </div>
                            </td>
                            <td className="px-4 py-4 font-black text-black text-sm">
                                ...
                                <div className="flex flex-wrap gap-1">
                                  {Array.isArray(campaign.niche) ? (
                                    campaign.niche.map((n: string, idx: number) => (
                                      <span
                                        key={idx}
                                        className="text-[9px] font-black text-gray-600 bg-gray-100 border border-gray-200 px-1.5 py-0.5 rounded uppercase tracking-widest"
                                      >
                                        {n}
                                      </span>
                                    ))
                                  ) : (
                                    <span className="text-[9px] font-black text-gray-600 bg-gray-100 border border-gray-200 px-1.5 py-0.5 rounded uppercase tracking-widest">
                                      {campaign.niche}
                                    </span>
                                  )}
                                </div>
            ...
            {completedCampaigns.length > 0 && (
              <div className="mt-8 bg-[#a3e635] border-2 border-black rounded-xl p-5 flex items-center gap-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                ...
                <p className="text-sm font-black text-black uppercase tracking-widest">
                  ...
                  <span className="text-black font-bold ml-2 opacity-80">
                    Invoices are available in each campaign's Deal Room.
                  </span>
                </p>
              </div>
            )}

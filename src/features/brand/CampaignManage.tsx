  ...
  if (!campaign) {
    return (
      <div className="min-h-screen bg-[#f9fafb] flex flex-col items-center justify-center p-6">
        ...
        <button onClick={() => navigate('/brand-dashboard')} className="btn-primary">
          Return to Dashboard
        </button>
      </div>
    );
      ...
      <div className="bg-white border-b-2 border-black pt-6 pb-6 px-4 sm:px-6 lg:px-8 shadow-[0px_4px_0px_0px_rgba(0,0,0,1)] relative z-10">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-center mb-4">
            <button
              onClick={() => navigate(-1)}
              className="text-black hover:text-indigo-600 text-xs font-black tracking-widest flex items-center transition-colors uppercase"
            >
            ...
            <div className="flex gap-3">
              <button
                ...
                className="bg-white text-black border-2 border-black px-4 py-2 rounded-lg text-xs font-black tracking-widest uppercase shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 active:translate-y-0 active:shadow-none transition-all flex items-center"
              >
              ...
              {canDelete && (
                <button
                  ...
                  className="bg-[#ef4444] text-white border-2 border-black px-4 py-2 rounded-lg text-xs font-black tracking-widest uppercase shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 hover:bg-[#dc2626] active:translate-y-0 active:shadow-none transition-all flex items-center"
                >
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mt-6">
            <div>
              <div className="flex flex-wrap items-center gap-2 mb-3">
                {Array.isArray(campaign.niche) ? (
                  campaign.niche.map((n: string, idx: number) => (
                    <span
                      key={idx}
                      className="bg-indigo-50 text-indigo-700 px-2 py-1 rounded text-[10px] font-black uppercase tracking-widest border border-indigo-200 shadow-sm"
                    >
                      {n}
                    </span>
                  ))
                ) : (
                  <span className="bg-indigo-50 text-indigo-700 px-2 py-1 rounded text-[10px] font-black uppercase tracking-widest border border-indigo-200 shadow-sm">
                    {campaign.niche}
                  </span>
                )}
              ...
              <p className="mt-2 text-sm font-bold text-gray-500 max-w-2xl">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6 border-b-2 border-black pb-4">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl lg:text-2xl font-black text-black tracking-tight uppercase">
              Creator Pipeline
            </h2>
          </div>
        ...
        {(activeTab === 'applicants' ? applicants : interested).length === 0 ? (
          <div className="bg-white border-2 border-dashed border-gray-300 rounded-xl p-10 text-center shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
            ...
            <h3 className="text-xl font-black text-black uppercase tracking-tight mb-2">
              {activeTab === 'applicants' ? 'No Applications Yet' : 'No Outbound Interests'}
            </h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {(activeTab === 'applicants' ? applicants : interested).map((creator) => (
              <div
                ...
                className="bg-white border-2 border-black rounded-xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col hover:-translate-y-0.5 hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] transition-all overflow-hidden"
              >
                <div className="p-5 flex items-start gap-4 bg-white">
                  <img
                    src={
                      ...
                      `https://ui-avatars.com/api/?name=${encodeURIComponent(creator.name)}&background=111827&color=fff`
                    }
                    alt={creator.name}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start">
                      ...
                      {creator.status === 'pending' && (
                        <span className="text-[9px] font-black text-black bg-[#fbbf24] px-1.5 py-0.5 rounded border-2 border-black ml-2 shrink-0 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] uppercase tracking-widest">
                          Applied
                        </span>
                      )}
                      {(creator.status === 'interested' || creator.type === 'outbound') && (
                        <span className="text-[9px] font-black text-black bg-[#60a5fa] px-1.5 py-0.5 rounded border-2 border-black ml-2 shrink-0 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] uppercase tracking-widest">
                          Outreach
                        </span>
                      )}
                      {creator.status === 'contracted' && (
                        <span className="text-[9px] font-black text-black bg-[#a3e635] px-1.5 py-0.5 rounded border-2 border-black ml-2 shrink-0 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] uppercase tracking-widest">
                          Contracted
                        </span>
                      )}
                    </div>
                    <p className="text-[11px] font-bold text-gray-500 mt-1 uppercase tracking-widest">
                      {Array.isArray(creator.niche) ? creator.niche.join(', ') : creator.niche} •{' '}
                      {creator.follower_count
                        ? `${(creator.follower_count / 1000).toFixed(1)}K subs`
                        : 'Creator'}
                    </p>
                    {creator.avg_views && (
                      <p className="text-[10px] font-black text-black mt-1.5 bg-gray-100 border border-gray-200 inline-block px-1.5 py-0.5 rounded uppercase tracking-widest">
                        {(creator.avg_views / 1000).toFixed(1)}K avg views
                      </p>
                    )}
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center p-4 backdrop-blur-sm animate-[fadeIn_0.2s_ease-out]">
          <div className="bg-white border-2 border-black p-6 md:p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] max-w-md w-full relative animate-[slideUp_0.2s_ease-out]">
            ...
            <p className="text-gray-600 font-medium mb-8">
              Are you sure you want to permanently delete this campaign? This action cannot be
              undone.
            </p>

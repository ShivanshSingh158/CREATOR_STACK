...
import {
  Building2, CheckCircle2, Shield, Wallet, ArrowRight, AlertCircle,
  User, Landmark, Star, Clock, Info, BadgeCheck,
} from 'lucide-react';
...
const TIERS = [
  {
    ...
    features: ['Personal PAN (format validated)', 'Bank account verification', '₹5,000 minimum escrow hold'],
  ...
  },
  {
    ...
    features: ['Corporate PAN (format validated)', 'GSTIN format check', '₹25,000 minimum escrow hold'],
  ...
  },
  {
    ...
    features: ['CIN + Corporate PAN (format validated)', 'GSTIN format check', 'LinkedIn company page', '₹1,00,000 minimum escrow hold'],
...
export default function BrandOnboarding() {
  ...
  const detectIndustryFromUrl = async (urlStr: string) => {
    try {
      ...
      for (const [ind, kws] of Object.entries(mapping)) {
        if (kws.some(kw => textToAnalyze.includes(kw))) { setIndustry(ind); break; }
      }
    } catch { /* ignore */ }
  };
  ...
  const handleStep3KYC = (e: React.FormEvent) => {
    e.preventDefault();
    let hasError = false;
    setPanError(''); setGstinError(''); setCinError('');
        ...
        <div className="flex items-center justify-center gap-0 mb-8">
          {STEPS.map((s, idx) => (
            <React.Fragment key={s.n}>
              ...
              {idx < STEPS.length - 1 && (
                <div className={`h-1 w-10 mb-5 mx-1.5 transition-all ${step > s.n ? 'bg-black' : 'bg-gray-200'}`} />
              )}
            </React.Fragment>
          ))}
        </div>
            ...
            <form onSubmit={handleStep1} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input type="text" required placeholder="e.g. Acme Technologies Pvt. Ltd." className={inputCls()} value={companyName} onChange={e => setCompanyName(e.target.value)} />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-black uppercase tracking-widest mb-1.5">Company Website *</label>
                  <input type="text" required placeholder="https://acme.com" className={inputCls()} value={website} onChange={e => setWebsite(e.target.value)} />
                  {detectingIndustry && <p className="text-[9px] font-black text-indigo-600 uppercase tracking-widest mt-1.5 animate-pulse">Detecting industry…</p>}
                </div>
                <div>
                  <label className="block text-[10px] font-black text-black uppercase tracking-widest mb-1.5">Industry</label>
                  <select className="w-full px-4 py-3 bg-white border-2 border-black rounded text-sm font-bold text-black focus:outline-none focus:border-indigo-600 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-all cursor-pointer appearance-none" value={industry} onChange={e => setIndustry(e.target.value)}>
                ...
                <div>
                  ...
                  <select className="w-full px-4 py-3 bg-white border-2 border-black rounded text-sm font-bold text-black focus:outline-none focus:border-indigo-600 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-all cursor-pointer appearance-none" value={budget} onChange={e => setBudget(e.target.value)}>
                    {BUDGETS.map(b => <option key={b}>{b}</option>)}
                  </select>
                </div>
              </div>

              <button type="submit" className="ml-auto flex items-center gap-2 bg-indigo-600 border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] text-white font-black py-3.5 px-10 rounded uppercase tracking-widest text-[10px] hover:bg-indigo-700 hover:-translate-y-0.5 active:translate-y-0 active:shadow-none transition-all">
        ...
        {!loading && step === 2 && (
          <div className="bg-white rounded-xl border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] p-8">
            <div className="flex items-center gap-4 mb-2">
              ...
              <div>
                <h2 className="text-lg font-black text-black uppercase tracking-tight">Choose Verification Tier</h2>
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mt-1">Your tier determines your trust badge and minimum escrow commitment</p>
              </div>
            </div>
            ...
            <div className="bg-amber-50 border-2 border-amber-400 rounded-lg p-3 mb-6 text-[10px] font-bold text-amber-800 uppercase tracking-widest leading-relaxed flex items-start gap-2">
              ...
              Your KYC documents will be reviewed by our team within 24–48 hours. Your account will be active immediately, but your verification badge appears after review.
            </div>

            <form onSubmit={handleStep2} className="space-y-4">
              {TIERS.map(t => (
                <label key={t.id} className={`flex items-start gap-5 p-5 rounded-xl border-2 cursor-pointer transition-all ${selectedTier === t.id ? 'border-indigo-600 bg-indigo-50 shadow-[4px_4px_0px_0px_rgba(79,70,229,1)]' : 'border-black bg-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:bg-gray-50 hover:-translate-y-0.5'}`}>
                  <input
                    type="radio"
                    name="tier"
                    value={t.id}
                    checked={selectedTier === t.id}
                    onChange={() => setSelectedTier(t.id as 'individual' | 'business' | 'enterprise')}
                    className="mt-1 shrink-0"
                  />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      <span className="text-sm font-black text-black uppercase tracking-tight">{t.label}</span>
                      <span className="text-[9px] font-black px-2 py-0.5 bg-black text-white rounded-full uppercase tracking-widest">{t.badge}</span>
                    </div>
                    <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3">{t.description}</p>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                      {t.features.map(f => (
                        <div key={f} className="flex items-center gap-1.5">
                          <CheckCircle2 className="w-3 h-3 text-indigo-600 shrink-0" />
                          <span className="text-[9px] font-bold text-gray-600 uppercase tracking-widest">{f}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  <div className="shrink-0 text-right">
                    <p className="text-[8px] font-black text-gray-500 uppercase tracking-widest">Min. Deposit</p>
                    <p className="text-sm font-black text-black">₹{t.minDeposit.toLocaleString('en-IN')}</p>
                  </div>
                </label>
              ))}

              {!selectedTier && <p className="text-[10px] font-black text-red-500 uppercase tracking-widest">Please select a verification tier to continue.</p>}

              <div className="flex gap-4 pt-2 justify-end">
                <button type="button" onClick={() => setStep(1)} className="px-8 py-3.5 text-[10px] font-black uppercase tracking-widest text-black bg-gray-100 border-2 border-black rounded shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:bg-gray-200 hover:-translate-y-0.5 active:translate-y-0 active:shadow-none transition-all">Back</button>
                <button type="submit" disabled={!selectedTier} className="flex items-center gap-2 px-10 py-3.5 text-[10px] font-black uppercase tracking-widest text-white bg-indigo-600 border-2 border-black rounded shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:bg-indigo-700 hover:-translate-y-0.5 active:translate-y-0 active:shadow-none transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                  Continue <ArrowRight className="w-4 h-4" />
                </button>
              </div>
            </form>
        ...
        {!loading && step === 3 && tier && (
          <div className="bg-white rounded-xl border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] p-8">
            <div className="flex items-center gap-4 mb-2">
              ...
              <div>
                <h2 className="text-lg font-black text-black uppercase tracking-tight">KYC Documents</h2>
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mt-1">{tier.label} tier — required fields for {tier.badge}</p>
              </div>
            </div>
            ...
            <div className="bg-yellow-50 border-2 border-yellow-400 rounded-lg p-3 mb-6 text-[10px] font-bold text-yellow-800 uppercase tracking-widest leading-relaxed flex items-start gap-2">
              ...
              We verify your PAN and documents for TDS compliance (Section 194J). Data is encrypted. Review by our compliance team takes 24–48 hours. <span className="font-black text-yellow-900">Documents are NOT auto-approved.</span>
            </div>
            ...
            <form onSubmit={handleStep3KYC} className="space-y-5">
              ...
              <div>
                ...
                <p className="text-[9px] font-bold text-gray-500 uppercase tracking-widest mt-1">Format: ABCDE1234F (5 letters + 4 digits + 1 letter)</p>
              </div>
              ...
              {(selectedTier === 'business' || selectedTier === 'enterprise') && (
                <div>
                  ...
                  />
                  {gstinError && <p className="text-[9px] font-black uppercase tracking-widest text-red-500 mt-1.5 flex items-center gap-1"><AlertCircle className="w-3 h-3" /> {gstinError}</p>}
                </div>
              )}
              ...
              {selectedTier === 'enterprise' && (
                <>
                  <div>
                    <label className="block text-[10px] font-black text-black uppercase tracking-widest mb-1.5">Corporate Identification Number (CIN) *</label>
                    <input
                      type="text"
                      required
                      ...
                      value={cin}
                      onChange={e => { setCin(e.target.value.toUpperCase()); setCinError(''); }}
                    />
                    {cinError && <p className="text-[9px] font-black uppercase tracking-widest text-red-500 mt-1.5 flex items-center gap-1"><AlertCircle className="w-3 h-3" /> {cinError}</p>}
                  </div>
                  <div>
                    <label className="block text-[10px] font-black text-black uppercase tracking-widest mb-1.5">LinkedIn Company Page URL *</label>
                    <input
                      type="text"
                      required
              ...
              <div className="bg-blue-50 border-2 border-blue-400 rounded-lg p-3 text-[10px] font-bold text-blue-800 uppercase tracking-widest leading-relaxed flex items-start gap-2">
                ...
                After submission, your documents enter our review queue. You can start creating campaigns immediately, but your verification badge will appear within 24–48 hours.
              </div>

              <div className="flex gap-4 pt-2 justify-end">
                <button type="button" onClick={() => setStep(2)} className="px-8 py-3.5 text-[10px] font-black uppercase tracking-widest text-black bg-gray-100 border-2 border-black rounded shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:bg-gray-200 hover:-translate-y-0.5 active:translate-y-0 active:shadow-none transition-all">Back</button>
          ...
          <div className="bg-white rounded-xl border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] p-8">
            <div className="flex items-center gap-4 mb-6">
              <div className="w-12 h-12 bg-[#a3e635] border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] rounded-lg flex items-center justify-center">
                {brandLogo ? <img src={brandLogo} alt="Logo" className="w-full h-full object-cover" referrerPolicy="no-referrer" /> : <Wallet className="w-5 h-5 text-black" />}
              </div>
              <div>
                <h2 className="text-lg font-black text-black uppercase tracking-tight">Escrow Wallet Setup</h2>
                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mt-1">Required: minimum ₹{tier.minDeposit.toLocaleString('en-IN')} for {tier.label} tier</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 mb-6 bg-[#a3e635] border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] rounded-lg p-4">
              ...
              <div>
                <p className="text-[10px] font-black text-black uppercase tracking-widest">KYC Submitted — Under Review</p>
                <p className="text-[9px] font-bold text-black/70 uppercase tracking-widest mt-0.5">PAN: {pan} • Documents sent to compliance team</p>
              </div>
            </div>
            ...
            {depositStage === 'idle' && (
              <div className="space-y-5">
                <div>
                  <label className="block text-[10px] font-black text-black uppercase tracking-widest mb-1.5">Deposit Amount (INR)</label>
                  <div className="grid grid-cols-4 gap-2 mb-3">
                    {[tier.minDeposit, tier.minDeposit * 2, tier.minDeposit * 4, tier.minDeposit * 10].map(p => (
                      <button
                        key={p}
                        type="button"
                  ...
                  {depositAmount && parseInt(depositAmount) < tier.minDeposit && (
                    <p className="text-[10px] font-black text-red-500 mt-1.5 uppercase tracking-widest flex items-center gap-1">
                      <AlertCircle className="w-3 h-3" /> Minimum deposit is ₹{tier.minDeposit.toLocaleString('en-IN')} for {tier.label} tier
                    </p>
                  )}
                </div>
                ...
                <div className="bg-indigo-50 border-2 border-indigo-400 rounded-lg p-4 text-[10px] font-bold text-indigo-800 uppercase tracking-widest leading-relaxed">
                  ...
                  These funds are held in escrow and can only be used for campaign payments. Unused funds are fully refundable anytime. You must fund escrow before initiating any deal room with a creator.
                </div>
            ...
            {depositStage === 'processing' && (
              <div className="text-center py-10">
                ...
                <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest animate-pulse">Processing Deposit…</p>
                <p className="text-[9px] font-bold text-gray-500 uppercase tracking-widest mt-2">Securing funds in your escrow wallet</p>
              </div>
            )}
            ...
            {depositStage === 'done' && (
              <div className="space-y-5">
                <div className="bg-[#a3e635] border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] rounded-lg p-5 flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-sm font-black text-black uppercase tracking-tight">₹{parseInt(depositAmount).toLocaleString('en-IN')} Deposited Successfully</p>
                    <p className="text-[10px] font-bold text-black/70 uppercase tracking-widest mt-1">Your escrow wallet is funded and ready</p>
                  </div>
                </div>

                <div className="grid grid-cols-3 gap-3">
                  {[
                    { label: 'Total Balance', value: `₹${parseInt(depositAmount).toLocaleString('en-IN')}` },
                    { label: 'Locked in Deals', value: '₹0' },
                    { label: 'Available', value: `₹${parseInt(depositAmount).toLocaleString('en-IN')}` },
                  ].map(item => (
                    <div key={item.label} className="bg-white border-2 border-black rounded-lg p-3 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] text-center">

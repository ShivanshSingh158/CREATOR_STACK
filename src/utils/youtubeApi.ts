...
function inferNicheFromTopics(topicCategories?: string[]): string {
  ...
  if (
    topicsStr.includes('video_game') ||
    topicsStr.includes('action_game') ||
    topicsStr.includes('role-playing_game') ||
    topicsStr.includes('strategy_game')
  ) {
  ...
  if (topicsStr.includes('technology') || topicsStr.includes('computer')) {
    if (topicsStr.includes('software') || topicsStr.includes('programming'))
  ...
  if (
    topicsStr.includes('finance') ||
    topicsStr.includes('business') ||
    topicsStr.includes('economics')
  ) {
...
export async function fetchYouTubeChannelMetrics(
  ...
  if (!YT_API_KEY || YT_API_KEY === 'YOUR_YOUTUBE_DATA_API_V3_KEY_HERE') {
    throw {
      type: 'INVALID_KEY',
      message: 'YouTube API key not configured. Add VITE_YOUTUBE_API_KEY to your .env file.',
    } as YouTubeAPIError;
  }
  ...
  if (!parsed) {
    throw {
      type: 'INVALID_URL',
      message: 'Could not parse YouTube channel URL. Please paste a valid channel link.',
    } as YouTubeAPIError;
  }
  ...
  if (!channelRes.ok) {
    ...
    if (channelRes.status === 403) {
      throw {
        type: err.error?.errors?.[0]?.reason === 'quotaExceeded' ? 'QUOTA_EXCEEDED' : 'INVALID_KEY',
        message: err.error?.message || 'API access denied.',
      } as YouTubeAPIError;
    }
    throw {
      type: 'NETWORK_ERROR',
      message: `YouTube API error: ${channelRes.status}`,
    } as YouTubeAPIError;
  }
  ...
  const bannerUrl = rawBanner
    ? `${rawBanner}=w1080-fcrop64=1,00005a57ffffa5a8-k-c0xffffffff-no-nd-rj`
    : '';
  ...
  if (videoIds.length > 0) {
    ...
    const allVideos = (videoData.items || []).map((v: any) => ({
    ...
    if (allVideos.length >= 10) {
      ...
      const recentAvg =
        allVideos.slice(0, sliceSize).reduce((s: number, v: any) => s + v.viewCount, 0) / sliceSize;
      const olderAvg =
        allVideos.slice(-sliceSize).reduce((s: number, v: any) => s + v.viewCount, 0) / sliceSize;
 ...
 */
export function youTubeMetricsToScraped(
  metrics: YouTubeChannelMetrics,
  niche: string,
  currentRate?: number,
): import('./valuationEngine').ScrapedMetrics {
  return {
    platform: 'YouTube',
